...
CREATE SEQUENCE amend_change_id_seq INCREMENT 1 START 100 OWNED BY Funding_Document_Change.amend_change_id;
...
CREATE TABLE Funding_Document_Change(
...
DELETE FROM AMENDMENT;
SELECT * FROM Funding_Document_Change;
DROP FUNCTION Fund_Doc_Change_Function CASCADE;

CREATE OR REPLACE FUNCTION Fund_Doc_Change_Function()
RETURNS TRIGGER LANGUAGE plpgsql
AS $trigfunc$
BEGIN

INSERT INTO Funding_Document_Change(alloc_chg_id, funding_document, old_recieved_amount, new_amend_id, new_recieved_amount, change_date)
VALUES(nextval('alloc_chg_id_seq'), NEW.funding_document, OLD.amendment_number, OLD.amendment_amount, NEW.amendment_number,NEW.amendment_amount, current_date);
WHERE Amendment.funding_document = Funding_Document_Change.funding_document;

RETURN NEW;
END;
$trigfunc$;
CREATE TRIGGER Allocation_Change_Trigger
BEFORE UPDATE OF amendment_amount ON Amendment 
FOR EACH ROW
EXECUTE PROCEDURE Fund_Doc_Change_Function();
...
DROP TRIGGER add_amendment(p_amend_id,p_funding_document,p_amendment_number,p_approval_date,p_amendment_amount)
(p_amend_id,p_funding_document,p_amendment_number,p_approval_date,p_amendment_amount)
...
CALL add_amendment(nextval('amend_id_seq'),588902345554, 001, CAST('15-Apr-22' AS DATE), 800000);
CALL add_amendment(nextval('amend_id_seq'),435664542132, 001, CAST('23-Sep-20' AS DATE), 600000);
CALL add_amendment(nextval('amend_id_seq'),435664542132, 002, CAST('10-Dec-20' AS DATE), 700500);
CALL add_amendment(nextval('amend_id_seq'),435664542132, 003, CAST('30-Jan-22' AS DATE), 805000);
...
0SELECT * FROM Allocation_Req_View;
SELECT * FROM Allocation_Request;
